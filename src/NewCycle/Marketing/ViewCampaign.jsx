...
const ViewCampaign = () => {
  ...
  async function formSubmit(e) {
    ...
    try {
      ...
      // let response = await axios.post(`${API_URL}/campaign`, postCampaign);
      // if (newProduct.length > 0 && response.data.message) {
      //   for (let i = 0; i < newProduct.length; i++) {
      //     let responseCampaign = await axios.post(`${API_URL}/post-lookups`, newProduct[i]);
      //   }
      // }
      ...
      const mockResponse = { data: { message: "Campaign created successfully" } };

      if (mockResponse.data.message) {
        setIsLoading(false);

        swal({
          text: "You started the campaign successfully (Static Mode)",
          icon: "success",
          button: false,
          timer: 1800,
        });
      ...
      } else if (mockResponse.data.error == "campaign name already exists!") {
        swal({
  ...
  // handle Getting data of api to new product - MODIFIED FOR STATIC APP
  async function getProductData(e, index) {
    ...
    if (!existLookupCode) {
      try {
        ...
        // let response = await axios.get(`${API_URL}/all-data/${e.target.value != "" && e.target.value}`);
        ...
        const mockResponse = {
          data: {
            description: [{
              Description: `Sample Product ${e.target.value}`,
        ...
        let list = [...newProduct];
        list[index].description = mockResponse.data.description[0].Description;
        list[index].product_id = mockResponse.data.description[0]["ID"];
