...
import { pqueueProducts } from '../../pqueueProducts';
...
const PlanningView = () => {
  ...
  const mockDepartments = [
    { ID: 1, DepartmentName: "Grocery" },
    { ID: 2, DepartmentName: "Fresh Food" },
    { ID: 3, DepartmentName: "Non-Food" },
    { ID: 4, DepartmentName: "Bakery" }
  ];
  ...
  const mockSuppliers = [
    { ID: 1, SupplierName: "ABC Suppliers Ltd." },
    { ID: 2, SupplierName: "XYZ Trading Co." },
    { ID: 3, SupplierName: "Fresh Foods Inc." },
    { ID: 4, SupplierName: "Global Distributors" }
  ];

// MODIFIED FOR SIMPLE STATIC APP
  useEffect(() => {
    setDepartments(mockDepartments);
    setCategories(mockCategories);
    setSuppliers(mockSuppliers);
    const product = pqueueProducts.find(item => item.id === Id);
    if (product) {
      setState({ ...state, loading: false, products: [product], errorMessage: '' });
      setPlanningApproval({ ...planningApproval, id: product.id, checkup: product.online === "0" });
    } else {
      setState({ ...state, loading: false, products: [], errorMessage: 'Product Not Found' });
    }
    ...
    if (postLog.action.length > 0 && planningApproval.pAccepted == 1) {
      setTimeout(async () => {
        ...
        try {
          ...
          // let response = await axios.post(`${API_URL}/planning`, {
          //   planningApproval: { ...planningApproval },
          //   catez: choosedCategories,
          // });
          ...
          console.log("mmmmmmmmmmmmm", mockResponse)

          if (mockResponse) {
            setIsLoadingApprove(false);
            swal({
              title: `Hi ${user.name}`,
              text: "Product has been approved (Static Mode)",
              icon: "success",
              button: false,
              timer: 1800,
            });
 ...
 */
  function formSubmitReject(e) {
    ...
    if (postLog.action.length > 0 && planningApproval.pRejected == 1) {
      setTimeout(async () => {
        try {
          // API calls commented out for static app
          // let response = await axios.post(`${API_URL}/planning`, planningApproval);
          // let responseLog = await axios.post(`${API_URL}/log`, postLog);
          ...
          if (mockResponse) {
            ...
            swal({
              title: `Hi ${user.name}`,
              text: "Product has been rejected (Static Mode)",
              icon: "info",
              button: false,
              timer: 1800,
            });
  ...
  // Hardcoded mock web categories for static app
  const mockWebCategories = [
    { CategoryID: "1", CategoryName: "Beverages", ParentCategoryID: "0" },
    { CategoryID: "2", CategoryName: "Snacks", ParentCategoryID: "0" },
    { CategoryID: "3", CategoryName: "Soft Drinks", ParentCategoryID: "1" },
    { CategoryID: "4", CategoryName: "Juices", ParentCategoryID: "1" },
    { CategoryID: "5", CategoryName: "Chips", ParentCategoryID: "2" },
    { CategoryID: "6", CategoryName: "Cookies", ParentCategoryID: "2" }
  ];

  const fetchCategories = async () => {
    try {
      // API call commented out for static app
      // const response = await axios.get(`${API_URL}/get-cats`);
      // const webcats = response.data.webcats;

      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 300));

      setWebCategories(mockWebCategories);
    } catch (error) {
...
By using setPlanningApproval to update the state of the component, React will re-render the component with the updated state, which will update the UI to reflect the user's input change. */
  ...
  // Safety checks to prevent undefined array errors
  const safeProducts = products || [];
  const safeWebCategories = webCategories || [];
  const safeChoosedCategories = choosedCategories || [];
                          ...
                          <div className="col-12 row justify-content-between g-4 mb-3">
                            ...
                            <div className="col-4">
                              ...
                              >
                                ...
                                {safeWebCategories.map((option, index) =>
                        ...
                        <div className="col-12 row justify-content-between mx-0 my-3">
                          <div className="  col-4 position-relative  ">
                            <MainButton
                              ...
                              disabled={
                                planningApproval && planningApproval.reason && planningApproval.reason.length > 0
                                  ? true
                                  : false
                              }
                              type="button"
