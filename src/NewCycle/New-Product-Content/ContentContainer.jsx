...
const ContentContainer = () => {
  ...
  // Hardcoded mock selected categories for static app
  const mockSelectedCats = [
    { id: 1, CategoryName: "Beverages", selected: true },
    { id: 2, CategoryName: "Snacks", selected: true },
    { id: 3, CategoryName: "Dairy", selected: false }
  ];

  useEffect(() => {
    // API call commented out for static app
    // axios.get(`${API_URL}/selected-cats/${Id}`)
    //   .then((response) => {
    ...
    setTimeout(() => {
      setCats(mockSelectedCats);
      setContentState((prev) => {
        return {
          ...prev,
          choosedCategories: [...mockSelectedCats],
        };
      });
    }, 300);
  }, [Id]);
  ...
  const [contentState, setContentState] = useState({
    ...
    log: {
      lookupcode: Id,
      action: `${user.name} Submitted the ${Param === "english"
        ? "English"
        : Param === "arabic"
          ? "Arabic"
          : Param === "finalContent"
...
Dietary: an array that holds the dietary data retrieved from */
  ...
  const mockContentData = {
    data: {
      ...
      dietary: [
        { DietryId: "1", EnglishName: "Vegetarian" },
        { DietryId: "2", EnglishName: "Vegan" },
        { DietryId: "3", EnglishName: "Gluten-Free" }
      ],
  ...
  const mockValuesRes = {
    data: {
      value: [
        { id: 1, value: "Small", type_id: 1 },
        { id: 2, value: "Large", type_id: 1 }
      ]
    }
  };
  ...
  useEffect(() => {
    const getContentData = async () => {
      try {
        ...
        //   Param === "existing"
        //     ? `${API_URL}/content/${Id}`
        //     : `${API_URL}/all-content/${Id}`
        // );
        // let typesRes = await axios.get(`${API_URL}/option-type`);
        // let valuesRes = await axios.get(`${API_URL}/option-value`);
        // let imagesResponse = await axios.get(`${API_URL}/photos/${Id}`);
        ...
        // Use mock data
        let contentResponse = mockContentData;
        let typesRes = mockTypesRes;
        let valuesRes = mockValuesRes;
        let imagesResponse = mockImagesResponse;
        let responseProduct = mockContentData;
...
If the API call to retrieve contentResponse data returns an array of length greater than 0, then it means that the content already exists and the function updates the state with the retrieved data. Otherwise, if the length of the array is 0, it means that the content does not exist and the function creates a default object to populate the contentData state property. */
  ...
  if (contentState.finalAllergenArray && contentState.finalAllergenArray.length > 0 && contentState.Allergens) {
    for (const item of contentState.finalAllergenArray) {
      if (!item) continue;
      
      const existAllergen = contentState.Allergens.find(
        (obj) => obj && obj.AllergenId && obj.AllergenId.toString() === item.toString().trim()
      );
      
      if (!existAllergen) continue;
      
      const existItem = contentState.choosedAllergen && contentState.choosedAllergen.find(
        (allergen) => allergen && allergen.AllergenId && allergen.AllergenId.toString() === existAllergen.AllergenId.toString()
      );
...
It updates the state using the setContentState() function. It spreads the previous state (prev) and updates the finalDietryArray and choosedDietry arrays by setting them to speratedList and list respectively. */
  ...
  if (contentState.finalDietryArray.length > 0) {
    ...
    for (const item of contentState.finalDietryArray) {
      ...
      const existDietry = contentState.Dietary && Array.isArray(contentState.Dietary)
        ? contentState.Dietary.find(
            (obj) => obj && obj.DietryId === item.trim()
          )
        : undefined;
      if (!existDietry) continue;
      // check if the object exists in choosedDietry
      const existItem = contentState.choosedDietry && Array.isArray(contentState.choosedDietry)
        ? contentState.choosedDietry.find(
            (itemD) => itemD && itemD.DietryId === existDietry.DietryId
          )
        : undefined;
      ...
      if (!existItem) {
        setContentState((prev) => {
          return {
            ...prev,
            choosedDietry: [
              ...(Array.isArray(prev.choosedDietry) ? prev.choosedDietry : []),
              {
...
The purpose of this function is to update the title property of each item in the contentState.options array based on the corresponding option in the contentState.optionValues array. This allows the title to be displayed in the UI instead of the RecID of the option. By memoizing the function, it will only be executed when necessary, which can improve performance. */
  ...
  async function formSubmit(e) {
    ...
    if (Param === "existing") {
      try {
        // API call commented out for static app
        // let contentResponse = await axios.post(`${API_URL}/update-content`, {
        //   ItemID: contentState.contentData[0].ItemID,
        ...
        const mockContentResponse = { data: { success: true } };

        if (mockContentResponse) {
          setContentState((prev) => {
            return {
              ...prev,
              awaiting: false,
          ...
          swal({
            title: `Hi ${user.name}`,
            text: "Data updated successfully (Static Mode)",
            icon: "success",
            button: false,
            timer: 1200,
          });
      ...
      try {
        // API calls commented out for static app
        // let queueResponse = await axios.post(`${API_URL}/update-queue`, contentState.queueUpdate);
        // let responseLog = await axios.post(`${API_URL}/log`, contentState.log);
        // let contentResponse = await axios.post(`${API_URL}/insert-update`, {
        ...
        const mockQueueResponse = { data: { success: true } };
        const mockResponseLog = { data: { success: true } };
        const mockContentResponse = { data: { success: true } };

        let queueResponse = mockQueueResponse;
        let responseLog = mockResponseLog;
        let contentResponse = mockContentResponse;
      ...
      if (e.target.innerText === "Approve") {
        ...
        if (contentState.buyingFlags.ready) {
          try {
            // API calls commented out for static app
            // let queueResponse = await axios.post(`${API_URL}/update-queue`, contentState.buyingFlags);
            // let responseLog = await axios.post(`${API_URL}/log`, contentState.log);
            ...
            if (mockQueueResponse && mockResponseLog) {
              setContentState((prev) => {
                return {
                  ...prev,
                  isLoadingApprove: false,
              ...
              swal({
                title: `Hi ${user.name}`,
                text: "Data Approved successfully (Static Mode)",
                icon: "success",
                button: false,
                timer: 1200,
              });
