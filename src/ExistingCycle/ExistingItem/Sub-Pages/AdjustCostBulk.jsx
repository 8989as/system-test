...
const AdjustCostBulk = () => {
  ...
  const handleFile = async (e) => {
    ...
    jsonData.filter((data) => data.itemlookupcode && data.new_cost).length > 0
      ...
      : swal({
        title: `Mr/Mrs ${JSON.parse(sessionStorage.getItem("userData"))?.name
          }`,
  ...
  async function formSubmit(e) {
    ...
    try {
      if (finalArr.length > 0) {
        ...
        // const response = await axios.post(`${API_URL}/update-cost`, finalArr);
        ...
        if (mockResponse.data.message) {
          swal({
            title: "Cost updated successfully (Static Mode)",
            icon: "success",
            button: false,
            timer: 1500,
          });
