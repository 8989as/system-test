...
const AdjustPrice = () => {
  ...
  const getTheProduct = useCallback(async () => {
    try {
      ...
      // Using the already modified fetchDataItem function which returns mock data
      ...
      setPriceState((prev) => {
        return {
          ...prev,
          loading: false,
          priceData: [{ ...response.data.description[0], 'lookupcode': response.data.description[0].ItemLookupCode || Id }],
        };
      });
    } catch (error) {
      swal({
...
getTheProduct() function is called when the component mounts and whenever getTheProduct changes
  ...
  async function formSubmit(e) {
    ...
    if (errorEdit) {
      ...
      try {
        ...
        // let response = await axios.post(`${API_URL}/update-price`, priceState.priceData);
        ...
        if (mockResponse) {
          ...
          swal({
            title: `Hi ${user.name}`,
            text: "Price updated successfully (Static Mode)",
            icon: "success",
            button: false,
            timer: 1200,
          });
  ...
  let responseLog = await axios.post(`${API_URL}/log", postLog);: Makes a POST request using Axios to the specified endpoint with the postLog data. The await keyword is used to indicate that the code should wait for the response from the server before proceeding.
  
  let response = await axios.post(`${API_URL}/update-price", priceState.priceData);: Makes another POST request using Axios to a different endpoint with the priceState.priceData data.
        ...
        if (e.target.value != "") {
          ...
          if (!existCode) {
            let response = await axios.get(
              `${API_URL}/all-data/${e.target.value}`
            );
