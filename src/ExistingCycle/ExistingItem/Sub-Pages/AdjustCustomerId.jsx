...
const AdjustCustomerId = () => {
  ...
  const handleFile = async (e) => {
    ...
    // let resFinal = await axios.get(`${API_URL}/customer-data`, { params: [...validatedArray] });
    ...
    const mockResFinal = {
      data: {
        'Customer data retrived successfully!': validatedArray.map(item => ({
          ...item,
          CustomerName: `Customer ${item.PhoneNumber}`,
          currentTier: 'Bronze'
        })),
        'Customer phone number not exist!': []
      }
    };

    console.log('resFinal ===> ', mockResFinal);
    setFileData(mockResFinal.data['Customer data retrived successfully!']);
    setFailedPhoneNum((pre) => [...pre, ...mockResFinal.data['Customer phone number not exist!']]);
  };
  ...
  async function formSubmit(e) {
    ...
    try {
      if (finalArr.length > 0) {
        // API call commented out for static app
        // const response = await axios.post(`${API_URL}/post-customer`, finalArr);
        ...
        if (mockResponse.data.message) {
          swal({
            title: "Customer updated successfully (Static Mode)",
            icon: "success",
            button: false,
            timer: 1500,
          });
