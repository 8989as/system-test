...
const AdjustDiscription = () => {
  ...
  const getTheProduct = useCallback(async () => {
    try {
      ...
      setProductUpdate({
        ...productUpdate,
        ItemID: response.data.description[0].ID || 1,
        ItemLookupCode: response.data.description[0].ItemLookupCode || Id,
  ...
  async function formSubmit(e) {
    ...
    try {
      ...
      // let response = await axios.post(`${API_URL}/description`, productUpdate);

      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1000));

      // Simulate successful response
      const mockResponse = { data: { description: true } };

      if (mockResponse.data.description) {
        setIsLoading(false);
        swal({
          title: `Hi ${user.name}`,
          text: "Description Updated successfully (Static Mode)",
          icon: "success",
          button: false,
          timer: 1200,
        });
