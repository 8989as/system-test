...
*/
  ...
  async function formSubmit(e) {
    ...
    try {
      ...
      // const response = await axios.post(`${API_URL}/update-kit-type`, { header: kitState.kitType, details: kitState.kitDetails });
      ...
      setIsLoading(false);
      swal({
        text: "Kit updated successfully (Static Mode)",
        icon: "success",
        button: false,
        timer: 1200,
      });
...
const response = await axios.post(`${API_URL}/update-kit-type", { header: kitState.kitType, details: kitState.kitDetails }): This sends a POST request to the specified API endpoint with the header and details data from the kitState object.
      ...
      if (kitState.kitDetails[index].Description == "") {
        ...
        if (e.target.value !== "" && e.target.value != Id) {
          let response = await axios.get(
            `${API_URL}/get-type/${e.target.value}`
          );
...
let response = await axios.get(`${API_URL}/get-type/${e.target.value}`)`: This sends a GET request to the specified API endpoint with the lookup code value.
