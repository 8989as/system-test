...
const AdjustWeb = () => {
  ...
  const getTheProduct = useCallback(async () => {
    try {
      ...
      // let catResponse = await axios.get(`${API_URL}/content/${Id}`);
      ...
      const mockCatResponse = {
        data: {
          ...
          Content: [{ EnName: "Sample English Name" }]
        }
      };
      let catResponse = mockCatResponse;
      setWebState((prev) => {
        return {
          ...prev,
          loading: false,
  ...
  async function formSubmit(e) {
    ...
    try {
      ...
      // let response = await axios.post(`${API_URL}/update-webitem`, [webState.webData]);
      ...
      if (mockResponse) {
        swal({
          text: "Status updated successfully (Static Mode)",
          icon: "success",
          button: false,
          timer: 1200,
        });
  ...
  const updateInput = (e) => {
    ...
    if (e.target.name == "webitem") {
      setPostLog((prev) => {
        return {
          ...prev,
          action: `${user.name} adjusted the item ${Id} Web Status to ${e.target.value == "1" ? "Web Item" : "Non Web Item"
            }  `,
        };
      });
    }
