...
const AdjustBypass = () => {
  ...
  // function for removing bypass - MODIFIED FOR STATIC APP
  const bypassFunHandler = async (ID) => {
    console.log('hello world ', ID);

    // API call commented out for static app
    // let res = await axios.post(`${API_URL}/remove-bypass/${ID}`);
    ...
    swal({
      text: "Item removed successfully from bypass (Static Mode)",
      icon: "success",
      button: false,
      timer: 1200,
    });

    console.log("Mock result: bypass removed");
    getTheProduct();
  };
  ...
  async function formSubmit(e) {
    ...
    try {
      ...
      // let response = await axios.post(`${API_URL}/update-bypass`, bypassState.bypass);
      ...
      swal({
        text: "Bypass Updated successfully (Static Mode)",
        icon: "success",
        button: false,
        timer: 1200,
      });
