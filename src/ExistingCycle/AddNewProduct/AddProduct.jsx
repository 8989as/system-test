...
const AddProduct = () => {
  ...
  // Calling APIs of departments and supplier - COMMENTED OUT FOR STATIC APP
  async function getDepartment() {
    try {
      ...
      // let responseCat = await axios.get(`${API_URL}/category`);
      // let responseSup = await axios.get(`${API_URL}/supplier`);
  ...
  // Submit the product - MODIFIED FOR STATIC APP
  async function formSubmit(e) {
    ...
    try {
      // API calls commented out for static app
      // let response = await axios.post(`${API_URL}/create-product`, productInfo.items);
      // let responseLog = await axios.post(`${API_URL}/log`, postLog);

      // Simulate successful response for static app
      const mockResponse = { data: { message: "success" } };

      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1000));

      if (mockResponse.data.message != "lookupcode already exists!") {
        setProductInfo((prev) => {
          return { ...prev, isLoading: false };
        });
        swal({
          text: "Added Item successfully (Static Mode)",
          icon: "success",
          button: false,
          timer: 1200,
        });
