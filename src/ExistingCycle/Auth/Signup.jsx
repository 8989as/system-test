...
const Signup = () => {
  ...
  // Hardcoded mock roles data for static app
  const mockRoles = [
    { id: 1, name: "Admin" },
    { id: 2, name: "Buying" },
    { id: 3, name: "Content" },
    ...
    { id: 6, name: "Media Buying" },
    { id: 7, name: "Buying/Assortment" },
    { id: 8, name: "Loyalty" }
  ];
  ...
  async function getTheContact() {
    try {
      ...
      setUser({ ...users, loading: false, roles: mockRoles });
    } catch (error) {
    ...
    if (validationResults.error) {
      ...
      try {
        if (users.user.role_id != null) {
          // API call commented out for static app
          // let response = await axios.post(`${API_URL}/auth/register`, users.user);
          ...
          const mockResponse = {
            data: {
              message: "User created successfully and waiting for manager approval"
            }
          };
