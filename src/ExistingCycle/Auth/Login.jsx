  ...
  async function formSubmut(e) {
    ...
    if (validationResults.error) {
      ...
      try {
        ...
        // let response = await axios.post(`${API_URL}/auth/login`, users.user);

        // Simulate API delay
        await new Promise(resolve => setTimeout(resolve, 1000));

        // Simulate successful login response for static app
        const mockResponse = {
          data: {
            user: {
              id: 1,
              ...
              status: "1",
              reason: "",
              created_at: "2024-01-01T10:03:19.357000Z",
              updated_at: "2024-01-01T10:03:19.357000Z"
            }
          }
        };
        ...
        if (mockResponse.data) {
          if (
            mockResponse.data.user.status != 0 &&
            mockResponse.data.user.status != 2
          ) {
            setIsLoading(false);
            if (props.setIsAuth) {
            ...
            swal({
              title: `Hi ${mockResponse.data.user.name}`,
              text: "You Logged in successfully (Static Mode)",
              icon: "success",
              button: false,
              timer: 1200,
            });
        ...
        swal({
          title: `Error `,
          text: "Login failed in static mode",
          icon: "error",
          button: false,
          timer: 1200,
        });
